...
* `filePath` | `filePaths`: filePath to be used for download a file or filePaths for uploading files. See [downloadFile](#downloadFile), [uploadFile](#uploadFile), [uploadFiles](#uploadFiles) for detailed information
...
Uploads a file saved on the device.  Takes am URL, parameters, headers, filePath, and the name of the parameter to pass the file along as.  See the [post](#post) documentation for details on what is returned on success and failure.
